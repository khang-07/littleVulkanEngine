...
LveDevice::LveDevice(LveWindow &window) : window{window} {
    createInstance(); // start vulkan lib
    setupDebugMessenger(); // validation layers
    createSurface(); // GLFW surface : connects w/ window
    pickPhysicalDevice(); // picks GPU
    createLogicalDevice(); // picks features of GPU
    createCommandPool(); // cmd buffer allocation?
}
...
void LveDevice::createInstance() {
    ...
    /* 
    Default: auto extensions = getRequiredExtensions();
    createInfo.enabledExtensionCount = static_cast<uint32_t>(extensions.size());
    createInfo.ppEnabledExtensionNames = extensions.data();
    */

    /*
    Fixes Error: validation layer: vkCreateInstance: Found drivers that contain devices which support 
    the portability subset, but the portability enumeration bit was not set! Applications that wish to 
    enumerate portability drivers must set the VK_INSTANCE_CREATE_ENUMERATE_PORTABILITY_BIT_KHR bit in 
    the VkInstanceCreateInfo flags and enable the VK_KHR_portability_enumeration instance extension.
    ...
    */
    ...
    std::vector<const char*> customExtensions = {
...
void LveDevice::createLogicalDevice() {
    ...
    /* 
    Fixes Error: validation layer: Validation Error: [ VUID-VkDeviceCreateInfo-pProperties-04451 ] 
    Object 0: handle = 0x10d6f9e10, type = VK_OBJECT_TYPE_PHYSICAL_DEVICE; | MessageID = 0x3a3b6ca0 | 
    vkCreateDevice: VK_KHR_portability_subset must be enabled because physical device VkPhysicalDevice 0x10d6f9e10[] 
    supports it The Vulkan spec states: If the VK_KHR_portability_subset extension is included in pProperties of 
    vkEnumerateDeviceExtensionProperties, ppEnabledExtensionNames must include "VK_KHR_portability_subset" 
    (https://vulkan.lunarg.com/doc/view/1.3.250.0/mac/1.3-extensions/vkspec.html#VUID-VkDeviceCreateInfo-pProperties-04451)
    */

    // start my code
    const std::vector<char*> test = {"VK_KHR_portability_subset",  "VK_KHR_swapchain"};
    // end my code
    createInfo.pEnabledFeatures = &deviceFeatures;
    createInfo.enabledExtensionCount = test.size(); // static_cast<uint32_t>(deviceExtensions.size());
    createInfo.ppEnabledExtensionNames = test.data(); // 
